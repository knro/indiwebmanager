...
def startINDIServer(port=7624):
    cmd = 'indiserver -p ' + \
        str(port) + ' -m 100 -v -f ' + indi_fifo + ' > /dev/null 2>&1 &'
    print cmd
    sys.stdout.flush()
...
def startINDIDriver(driver):
    ...
    if "@" in driver.binary:
        ...
        cmd = 'echo "start ' + driver.binary + ' -n \\"' + \
            driver.label + '\\""  > ' + indi_fifo
    print cmd
    sys.stdout.flush()
    subprocess.call(cmd, shell=True)
...
def setINDIValue(deviceName, propertyName, elementName, value):
    cmd = "indi_setprop " + deviceName + "." + \
        propertyName + "." + elementName + "=" + value
    # print cmd
    sys.stdout.flush()
    subprocess.call(cmd, shell=True)
...
def getRunningDrivers():
    runningDrivers = [proc.name() for proc in psutil.process_iter()
                      if proc.name().startswith("indi_")]
