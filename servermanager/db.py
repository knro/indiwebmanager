...
def get_profile_drivers_labels(profile_name):
    ...
    cursor = conn.execute(
        "SELECT label FROM driver WHERE profile=(SELECT id FROM profile WHERE name='" + profile_name + "')")
    results = cursor.fetchall()
    return results
...
def get_profile_custom_drivers(profile_name):
    cursor = conn.execute(
        "SELECT drivers FROM custom WHERE profile=(SELECT id FROM profile WHERE name='" + profile_name + "')")
...
def delete_profile(profile_name):
    try:
        conn.execute(
            "DELETE FROM driver WHERE profile=(SELECT id FROM profile WHERE name='" + profile_name + "')")
...
def add_profile(profile_name):
    try:
        conn.execute(
            "INSERT INTO profile (name) VALUES('" + profile_name + "')")
    except Exception:
...
def update_profile(name, data):
    ...
    try:
        if (autostart != -1):
            conn.execute("UPDATE profile SET autostart=" +
                         str(autostart) + " WHERE id =" + str(profile_id))
        if (port != -1):
            conn.execute("UPDATE profile SET port=" +
                         str(port) + " WHERE id =" + str(profile_id))
    except Exception:
...
def save_profile_drivers(profile_name, drivers):
    cursor = conn.execute(
        "SELECT id FROM profile WHERE name='" + profile_name + "'")
    result = cursor.fetchone()
    ...
    profile_id = result['id']
    cursor = conn.execute(
        "DELETE FROM driver WHERE profile =" + str(profile_id))
    cursor = conn.execute(
        "DELETE FROM custom WHERE profile =" + str(profile_id))
    for driver in drivers:
        print (driver)
        try:
            if "label" in driver:
                conn.execute("INSERT INTO driver (label, profile) VALUES('" +
                             driver["label"] + "'," + str(profile_id) + ")")
            elif "custom" in driver:
                conn.execute("INSERT INTO custom (drivers, profile) VALUES('" +
                             driver["custom"] + "'," + str(profile_id) + ")")
        except Exception:
